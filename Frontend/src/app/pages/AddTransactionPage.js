...
const AddTransactionPage = () => {
  ...
  const handleMainCategory = (e) => {
    ...
    setCustomSub('');
    setIsOtherSub(false);
    // if "Other" main category, just set category directly
    if (val === 'Other') {
  ...
  const handleSubCategory = (e) => {
    ...
    if (val === '__other__') {
      setIsOtherSub(true);
      setSubCategory('');
      setFormData({ ...formData, category: mainCategory });
    } else {
      setIsOtherSub(false);
      setSubCategory(val);
      setFormData({ ...formData, category: val });
  ...
  const handleCustomSub = (e) => {
    ...
    setFormData({ ...formData, category: e.target.value || mainCategory });
  };
      ...
      <main className="dashboard-container">
        <div className="card" style={{ maxWidth: '600px', margin: '0 auto' }}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            {/* show final category value */}
            {formData.category && (
              <div style={{ marginBottom: 12, fontSize: 12, color: '#888' }}>
                Saving as: <strong style={{ color: '#059669' }}>{formData.category}</strong>
              </div>
            )}
